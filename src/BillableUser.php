...
class BillableUser
{
    ...
    {
        $braintree_customer_id = $this->getBraintreeCustomerId($user);

        try {
            $result = $this->braintreeApiService->getGateway()->customer()->find($braintree_customer_id);
        } catch (\Throwable $th) {
    ...
    public function preventDuplicatePaymentMethods(User $user, $payment_method)
    {
        ...
        'field_duplicate_user' => $uids,
            ...
            $this->messenger->addError($this->bcConfig->get('duplicate_payment_method_message'));
            $this->logger->error(
                'Duplicate payment method. User account uids with this payment method: %uids',
